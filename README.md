...
```text
读 task_state
-> 读 agent_runtime（能力目录、TaskPlan、verdict、resume）
-> 读取当前工作流节点
-> 执行有限步骤
-> 记录观察
-> 写入 verifier verdict
...
- **工作流画布**：独立全屏工作台，参考 Dify Start/End 节点、n8n 节点连接和 React Flow handle/edge 交互，把入口、确认、隔离、计划、工具/API、审批、任务记忆、出口回流做成拼图式节点。画布就是整个页面背景，不再作为页面里的嵌入模块；普通顶部 Token/刷新/反馈栏会隐藏，系统按钮以悬浮 HUD 叠在画布上。画布支持左侧导航收缩/展开小按钮、右侧模块素材抽屉、节点编辑弹窗、鼠标滚轮缩放、空白处拖拽平移、节点边缘圆点拖拽/点选连线、彩色节点与同色连线、小地图、右键复制/删除、静态检查、预跑诊断和 JSON 导入/导出兜底。
...
`tick` 现在会先进入轻量 `WorkflowRuntime`：入口确认、隔离快照、任务记忆检查点、心跳检查点等确定性节点由插件运行时直接推进；并行分支会优先走真实并发执行；只有计划、条件分支、工具/API、验证、归档这类需要开放式判断的节点才交给 ReAct/tool-loop 单步处理。这让画布从“提示词说明”进一步变成任务游标和节点执行器。
