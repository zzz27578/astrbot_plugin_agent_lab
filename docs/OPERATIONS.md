...
- 工作流节点、节点素材、拖拽位置和连线在独立“工作流画布”页编辑；“任务模式设置”页只保留触发、入口/出口话术、隔离、记忆、审批、心跳和提示词。画布页是全屏背景工作台，普通顶栏/Token/刷新/连接反馈隐藏，当前方案、保存、检查、预跑和自动布局以悬浮 HUD 叠在画布上。画布支持左侧导航收缩/展开小按钮、右侧素材抽屉、节点编辑弹窗、圆点拉线、彩色连线、右键复制/删除、连线删除、空白处拖拽平移、滚轮缩放、自动布局、检查工作流和预跑诊断，原始 JSON 只作为高级导入/导出兜底。
- AstrBot 插件、自定义 API、白名单工具都可以从画布工具箱添加为工作流模块；模块节点会保存 `plugin_name`、`api_id` 或 `tool_name`，并可写入节点提示词、条件、路径/URL、上游变量、输出变量、记忆标签和并行分组。
- 任务记忆有独立页面，可查看来源任务、标签、普通模式可读状态、续写草稿和归档回档入口；可把记忆或归档任务带入下一次任务 brief。
...
- Canvas 的“检查工作流”和“预跑诊断”应能发现缺少入口、出口、入口摘要、隔离快照、任务记忆、审批闸门、不可达节点、未绑定 API、文件/文档类节点缺少输入、危险动作缺少审批、被隔离插件和不在白名单中的工具。
