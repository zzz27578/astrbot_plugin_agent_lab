...
> 2026-06-11 第三方审计闭环补正：针对 GPT 审计发现的两处真实闭环缺口已修复——(1) 后端校验器 `_workflow_report`(main.py ~8655) 原按字面 `confirm_entry`/`restore_isolation` 判断，合并到 `listen_message`/`global_control` 后会误报 `missing_entry_confirmation`/`missing_isolation_snapshot`，现改为接受合并后节点（`confirmation_actions={confirm_entry,listen_message}`、`isolation_actions={restore_isolation,global_control}`）；(2) 监听节点的「进入流程前需要确认」复选框原只写 `node.require_confirmation`、不连 `entry_policy`，现用声明式 `syncPath` 双向绑定到 `entry_policy.require_confirmation`（render 读、save 写回 `agentFieldByPath`/`setAgentFieldByPath`），与全局「触发/生效范围」抽屉同源。GPT 另指 P1（前端缺 poke/notice/silent_global 触发类型）经核对不成立：`WORKFLOW_TRIGGER_TYPES`(app.js:147) 三者俱全且已渲染为复选框并写回。校验：node --check / compileall 通过，path-helper 4/4、校验器 5 场景仿真全通过。

> 2026-06-11 二次深审补正（前端字段↔后端真消费，全部闭环）：复审发现多个「前端能填但后端不读」的悬空字段，已逐个前后端打通——(1) 上下文压缩 context_bridge / 记忆过滤器原映射的 `summarize_entry` 字段（entry_summary_turns/compression_strategy/compression_max_tokens/preserve_keywords）经 `syncPath` 接入 `memory_policy.*`，后端 `AgentSummarizer.summarize_entry(policy=…)` 真正按策略压缩（轮数窗口、保留关键词、策略提示、token 上限），调用点 main.py 传 `spec.memory_policy`；(2) 记忆过滤器 memory_filter 改为独立 action + 职责表单（准入白/黑名单、屏蔽日常记忆、回流暴露范围），新增执行器 `_execute_memory_filter_node` 写 `workflow_data.memory_filter`，`_execute_retrieve_memory_node` 真按白/黑名单 + block_daily 过滤；(3) 错误捕获 catch_error 加 `disposition`（retry 走重试边 / pause 真暂停任务 / 其余走错误边），执行器消费；(4) 人工接管 handoff 加 `handoff_mode` 四选项，`_execute_wait_node` 据此定 `wait_reason` 与 source；(5) 并行分支 `branches` 注入并行 shared_instruction；心跳节点暴露 mode/cron/失败上限并 `syncPath` 写回 `heartbeat_policy.*`（调度器消费）。syncPath 已从 checkbox 泛化到所有字段类型（render 读、save 写）。UI：新增「填空题」表单美化（输入焦点环、hover、复选框品牌色、`.simple-note` 提示卡、`.simple-editor-lead`、`.req` 必填标）。验证：app.js `node --check` 通过、后端 `compileall` 通过、字段定义+保存仿真 22/22、syncPath round-trip 7/7、每个新字段均有后端消费点（grep 确认）。注意：编辑期间发现 Edit 工具写 app.js 在磁盘被硬截断在 426247 字节（基础设施层，非代码问题），已用 bash 重写修复并 `node --check` 通过；**今后 app.js 务必用 bash 原子写，勿用会话缓存行号直接 Edit**。
